...
import { 
    ...
    ChevronRight, Send, Zap, RefreshCw, Key, Layers, Link as LinkIcon, Copy
} from 'lucide-react';
...
const EgyDeadScraperPage: React.FC = () => {
    const queryClient = useQueryClient();
    const [activeTab, setActiveTab] = useState<'discovery' | 'linker'>('discovery');
    const [url, setUrl] = useState('https://tv7.egydead.live/');
    ...
    const { data: animesRes, isLoading: isAnimesLoading } = useQuery({
        queryKey: ["animes-search-egydead", animeSearch],
    ...
    const { data: episodesRes } = useQuery({
        queryKey: ["episodes-egydead-linker", selectedAnimeId],
    ...
    const { data: serversRes } = useQuery({
        queryKey: ["servers-egydead-linker"],
    ...
    const scrapeMutation = useMutation({
        ...
        onSuccess: (data: ScrapeResult) => {
            if (data.success) {
                ...
                toast.success(`تم جلب ${data.count} عنصر من إيجي ديد بنجاح!`);
                if (!animeName && data.title) {
                    setAnimeName(data.title.split(':')[0].trim());
                }
            } else {
                toast.error('فشل جلب البيانات من إيجي ديد');
            }
        },
        onError: (error: any) => {
    ...
    const deepImportMutation = useMutation({
        ...
        onSuccess: (data) => {
            toast.success(data.message || 'تم السحب العميق من إيجي ديد بنجاح!');
    ...
    const batchScrapeMutation = useMutation({
        mutationFn: async (targetUrl: string) => {
            const res = await api.post('/scraper/egydead-batch', { url: targetUrl });
            return res.data;
        },
        onSuccess: (data) => {
            setBatchResult(data);
            if (data.success) {
                toast.success(`تم جلب ${data.totalEpisodes} حلقة وسيرفراتها بنجاح!`);
                if (data.episodes?.length > 0) {
                    setExpandedEpisode(data.episodes[0].episodeNum);
                }
            } else {
                toast.error('فشل الجلب المتوازي من إيجي ديد');
            }
        },
        onError: (error: any) => {
            toast.error(error?.response?.data?.error || 'حدث خطأ أثناء الجلب المتوازي');
        }
    });
    ...
    const publishAllMutation = useMutation({
        mutationFn: async () => {
            if (!batchResult || !selectedAnimeId) {
                throw new Error("تأكد من اختيار الأنمي المراد ربط الحلقات به");
            }
            ...
            for (const ep of batchResult.episodes) {
                ...
                if (dbEp) {
                    ...
                    if (embedLinks.length > 0) {
                        try {
                            const newServers = embedLinks.map(link => ({
                                episode_id: dbEp.id,
                                language: "ar",
                                name: link.host?.toUpperCase() || link.title?.toUpperCase() || "EgyDead",
                                url: link.embedUrl,
                                type: "embed"
                            }));
                            ...
                            await api.put(`/episodes/${dbEp.id}`, {
                                ...dbEp,
                                servers: [...(dbEp.servers || []), ...newServers],
                                is_published: true
                            });
                            successCount++;
        ...
        onSuccess: (count) => {
            toast.success(`تم نشر ${count} حلقة بأسماء سيرفراتها الأصلية بنجاح!`);
            setIsPublishingAll(false);
            queryClient.invalidateQueries({ queryKey: ["episodes-egydead-linker", selectedAnimeId] });
        },
        onError: (error: any) => {
    ...
    const handleDownloadAll = async () => {
        ...
        const namePrefix = animeName || 'egydead-images';
        ...
        <div className="min-h-screen bg-[#0f0f0f] text-white p-4 md:p-8" dir="rtl">
            ...
            <div className="max-w-6xl mx-auto mb-10">
                <div className="flex items-center gap-4 mb-8">
                    <div className="w-14 h-14 bg-gradient-to-br from-red-600 via-orange-500 to-yellow-500 rounded-2xl flex items-center justify-center shadow-2xl shadow-red-900/40 border border-white/10">
                        <ImageIconLucide className="w-8 h-8 text-white" />
                    </div>
                    <div>
                        <h1 className="text-4xl font-black bg-gradient-to-r from-red-400 via-orange-400 to-yellow-400 bg-clip-text text-transparent tracking-tight">
                            ساحب إيجي ديد الذكي
                        </h1>
                        <p className="text-xs text-gray-400 font-medium mt-1">نظام الربط المتوازي واستخراج المحتوى المتطور</p>
                    </div>
                </div>
                ...
                <div className="flex gap-2 p-1 bg-[#1a1a1a] border border-white/5 rounded-2xl w-fit mb-8">
                    <button 
                        ...
                        className={cn(
                            ...
                            activeTab === 'discovery' ? "bg-red-600 text-white shadow-lg" : "text-gray-500 hover:text-gray-300"
                        )}
                    >
                    ...
                    <button 
                        ...
                        className={cn(
                            "px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2",
                            activeTab === 'linker' ? "bg-red-600 text-white shadow-lg" : "text-gray-500 hover:text-gray-300"
                        )}
                    >
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
                                <div className="lg:col-span-6 space-y-2">
                                    <label className="flex items-center gap-2 text-sm font-medium text-gray-400 px-1">
                                        <Globe className="w-4 h-4" /> رابط إيجي ديد المستهدف
                                    </label>
                                    <input
                                        ref={inputRef}
                                        type="url"
                                        ...
                                        placeholder="https://tv7.egydead.live/"
                                        className="w-full bg-[#252525] border border-white/10 rounded-2xl text-white py-4 px-6 outline-none focus:border-red-500 transition-all shadow-inner"
                                        dir="ltr"
                                    />
                                </div>
                                ...
                                <div className="lg:col-span-2 space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-[#252525] border border-white/10 rounded-2xl text-white py-4 px-6 outline-none focus:border-red-500 transition-all text-center"
                                    />
                                </div>
                                ...
                                <div className="lg:col-span-4 flex gap-3">
                                    ...
                                    >
                                        {scrapeMutation.isPending ? (
                                            ...
                                            <><Search className="w-6 h-6" /> ابدأ اكتشاف إيجي ديد</>
                                        )}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="mt-8 pt-8 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        placeholder="مثال: Predator Badlands"
                                        className="w-full bg-[#252525] border border-white/10 rounded-2xl text-white py-3 px-6 outline-none focus:border-red-500 transition-all"
                                    />
                                </div>
                        ...
                        {scrapeMutation.isPending ? (
                            <div className="mt-12 text-center py-20 bg-[#1a1a1a] rounded-[3rem] border border-white/5">
                                <div className="relative w-24 h-24 mx-auto mb-6">
                                    ...
                                    <div className="relative w-24 h-24 bg-gradient-to-br from-red-600/20 to-orange-500/20 rounded-full flex items-center justify-center border border-white/10">
                                        <ImageIconLucide className="w-10 h-10 text-red-500 animate-pulse" />
                                    </div>
                                </div>
                                <h3 className="text-2xl font-bold text-white mb-2">جاري قراءة إيجي ديد...</h3>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="flex items-center gap-1 bg-[#1a1a1a] p-1 rounded-xl border border-white/5 mr-4">
                                            <button 
                                                ...
                                                className={cn("p-2 rounded-lg transition-all", viewMode === 'grid' ? "bg-red-600 text-white" : "text-gray-500 hover:text-gray-300")}
                                            >
                                                <ImageIconLucide className="w-4 h-4" />
                                            </button>
                                            <button 
                                                onClick={() => setViewMode('list')}
                                                className={cn("p-2 rounded-lg transition-all", viewMode === 'list' ? "bg-red-600 text-white" : "text-gray-500 hover:text-gray-300")}
                                            >
                                        ...
                                        <button 
                                            ...
                                            className="text-xs font-bold bg-red-600/20 hover:bg-red-600/40 text-red-400 px-4 py-2 rounded-xl border border-red-500/20 transition-all"
                                        >
                                            تحديد الكل
                                        </button>
                                        <button 
                                                ...
                                                <div 
                                                    ...
                                                    className={cn(
                                                        ...
                                                        selectedImages.has(item.url) 
                                                            ? "border-red-500 scale-[0.98] shadow-2xl shadow-red-500/20" 
                                                            : "border-transparent hover:border-white/20",
                                                        activeAnime?.url === item.url && "ring-4 ring-red-500/30"
                                                    )}
                                                >
                                                    <img 
                                                        src={item.url} 
                                                    ...
                                                    <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-2 group-hover:translate-y-0 transition-transform">
                                                        ...
                                                        <div className="flex items-center justify-between mt-1">
                                                            <p className="text-[8px] text-red-400 font-medium">{item.status}</p>
                                                    ...
                                                    {selectedImages.has(item.url) && (
                                                        <div className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 shadow-lg ring-2 ring-white/20">
                                                ...
                                                {scrapeResult.data.map((item, idx) => (
                                                    <div 
                                                        ...
                                                        className={cn(
                                                            ...
                                                            activeAnime?.url === item.url ? "border-red-500 bg-red-500/5 shadow-lg" : "border-white/5 hover:border-white/10"
                                                        )}
                                                    >
                                                        ...
                                                        <div className="flex flex-col justify-between py-2">
                                                            <button 
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    selectedImages.has(item.url) ? "bg-red-600 text-white" : "bg-white/5 text-gray-600 hover:bg-white/10"
                                                                )}
                                                            >
                                                ...
                                                <div className="relative z-0 space-y-6">
                                                    ...
                                                    <div className="space-y-4 pt-2 border-t border-white/5">
                                                        <div className="space-y-2">
                                                            <label className="text-[10px] font-bold text-gray-400 flex items-center gap-2">
                                                                <ExternalLink className="w-3 h-3 text-red-500" /> رابط العمل على إيجي ديد (للسحب العميق)
                                                            </label>
                                                            <input 
                                                                type="url"
                                                                ...
                                                                dir="ltr"
                                                                placeholder="https://tv7.egydead.live/..."
                                                                className="w-full bg-black/30 border border-white/10 rounded-xl py-2 px-3 text-xs text-red-300 outline-none focus:border-red-500/50 transition-all font-mono"
                                                            />
                                                        </div>
                                                        ...
                                                        <button 
                                                            ...
                                                            className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-red-600 via-orange-600 to-red-700 hover:brightness-110 text-white py-4 rounded-2xl font-bold transition-all shadow-xl shadow-red-900/30 active:scale-95 disabled:opacity-50"
                                                        >
                                                            {isDeepImporting ? (
                                                            ...
                                                            تنزيل ذكي (سحب حلقات إيجي ديد + السيرفرات)
                                                        </button>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                                                            ...
                                                            <p className="text-sm font-black text-red-400">{activeAnime.episodes || 'غير معروف'}</p>
                                                        </div>
                                                        <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                            ...
                            <div className="mt-12 text-center py-32 bg-[#1a1a1a] rounded-[3rem] border border-white/5 relative overflow-hidden">
                                ...
                                <div className="max-w-md mx-auto space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <h3 className="text-2xl font-bold text-white">ابدأ بسحب إيجي ديد الآن</h3>
                                        <p className="text-gray-500 font-medium">
                                            ضع رابط الموقع (TV7.EgyDead.live) وسنقوم باستخراج جميع البوسترات والبيانات والحلقات دفعة واحدة بنظام ذكي.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between bg-[#1a1a1a] p-6 rounded-[2rem] border border-white/5 shadow-2xl gap-6">
                             <div className="flex items-center gap-6 flex-1 w-full">
                                <div className="space-y-2 flex-1 relative">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1 flex items-center gap-2">
                                       <Search className="w-3 h-3 text-red-500" /> ابحث عن أنمي للربط
                                    </label>
                                    <div className="relative group">
                                         <input 
                                            type="text"
                                            ...
                                            className="w-full bg-[#252525] border border-white/10 rounded-2xl py-3.5 pr-12 pl-4 text-sm outline-none focus:border-red-500/50 transition-all font-bold"
                                        />
                             ...
                             {batchResult && (
                                <button 
                                    ...
                                    disabled={!selectedAnimeId || isPublishingAll}
                                    className="px-10 py-4 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white rounded-2xl font-black text-lg shadow-xl shadow-green-900/20 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-3"
                                >
                        ...
                        <div className="bg-[#1a1a1a] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
                             <div className="flex flex-col lg:flex-row gap-6">
                                <div className="flex-1 space-y-2">
                                     <label className="text-xs font-bold text-gray-400 px-1">رابط السحب المباشر من إيجي ديد</label>
                                     <input 
                                        type="url"
                                        dir="ltr"
                                        value={url}
                                        onChange={(e) => setUrl(e.target.value)}
                                        placeholder="https://tv7.egydead.live/season/..."
                                        className="w-full bg-[#252525] border border-white/10 rounded-2xl py-4 px-6 text-sm text-red-400 outline-none focus:border-red-500 transition-all font-mono"
                                    />
                                </div>
                                <button 
                                    onClick={() => batchScrapeMutation.mutate(url)}
                                    disabled={batchScrapeMutation.isPending || !url}
                                    className="lg:mt-6 px-12 py-4 bg-red-600 hover:bg-red-500 text-white rounded-2xl font-black text-lg transition-all shadow-xl shadow-red-900/30 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
                                >
                                    ...
                                    {animes.map((a: any) => (
                                        <button 
                                            ...
                                            className={cn(
                                                ...
                                                selectedAnimeId === a.id ? "bg-red-600 border-red-500 shadow-xl" : "bg-white/5 border-white/5 hover:bg-white/10"
                                            )}
                                        >
                                            ...
                                            <div className="flex-1 py-1">
                                                ...
                                                <p className={cn("text-[10px] mt-1", selectedAnimeId === a.id ? "text-red-100" : "text-gray-500")}>{a.status} • {a.type}</p>
                                            </div>
                                                ...
                                                return (
                                                    <div 
                                                        ...
                                                        className={cn(
                                                            ...
                                                            expandedEpisode === ep.episodeNum ? "border-red-500 shadow-xl" : "border-white/5"
                                                        )}
                                                    >
                                                        <div 
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-4">
                                                                <div className={cn(
                                                                    ...
                                                                    matchInDB ? (isPublished ? "bg-green-600 text-white" : "bg-red-600 text-white") : "bg-white/5 text-gray-600"
                                                                )}>
                                                                    {ep.episodeNum}
                                                                </div>
                                                                <div>
                                                                ...
                                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                                                                    {ep.links.map((link, idx) => (
                                                                        <div key={idx} className="bg-black/40 p-4 rounded-xl border border-white/5 space-y-3 group/item">
                                                                            <div className="flex items-center justify-between">
                                                                                <span className="text-[10px] font-black bg-red-600/20 text-red-400 px-3 py-1 rounded-md border border-red-600/20 uppercase">
                                                                                    {link.title}
                                                                                </span>
                                                                            ...
                                                                            <div className="flex items-center gap-2 bg-red-500/5 border border-red-500/10 p-2.5 rounded-lg group-hover/item:border-red-500/30 transition-all">
...
export default EgyDeadScraperPage;
